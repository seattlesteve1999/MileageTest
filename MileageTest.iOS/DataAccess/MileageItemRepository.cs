...
using MileageManagerForms.iOS.Utilities;
using SQLite;
using System;
...
namespace MileageManagerForms.iOS.DataAccess
{
    public class MileageItemRepository
    {
        ...
        public async Task<List<MileageTableDefination>> GetMileageData(int autoId) => await sqlConnection.QueryAsync<MileageTableDefination>("select * from MileageData where Carid = ? ", autoId);
        ...
        public async Task<List<MileageTableDefination>> GetMileageDisplayDataByDate(int autoId, string month, int year)
        {
            ...
            return await sqlConnection.QueryAsync<MileageTableDefination>("select * from MileageData where Carid = ? AND (substr(StrDate, 1, 2) = ? AND substr(StrDate, 7,4) = ?)", autoId, newMonth, year.ToString());
        }
        ...
        internal async Task<int> AddMileageData(MileageTableDefination todoItem)
        {
            return await sqlConnection.InsertAsync(todoItem);
        }

        internal int AddMileageDataSync(MileageTableDefination todoItem)
        {
        ...
        internal async Task<int> AddAutoData(AutoTableDefination autoData)
        {
            return await sqlConnection.InsertAsync(autoData);
        }
